...
Управление взглядом камеры осуществляется при помощи мыши. Скорость передвижения камеры управляется колесом прокрутки (при зажатии клавиши `LeftAlt` будет изменяться FOV).

Для отображения курсора необходимо зажать клавишу `LeftControl`.
